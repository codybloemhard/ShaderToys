...
float scene_dist(vec3 p){
    ...
    float leg0l = capsule(q-vec3(0,.1,z+.3), .5, .05);
    float leg0r = capsule(q-vec3(0,.1,z-.3), .5, .05);
    float foot0l = sphere(q,vec3(-.15,0,z+.3), .1);
    float foot0r = sphere(q,vec3(-.15,0,z-.3), .1);
    q.x = mod(max(0.,p.x + 0.9),c)-0.5*c;
    float body1 = sphere(q,vec3(0,1,z), .6);
    float leg1l = capsule(q-vec3(0,.1,z+.3), .5, .05);
    float leg1r = capsule(q-vec3(0,.1,z-.3), .5, .05);
    float foot1l = sphere(q,vec3(-.15,0,z+.3), .1);
    float foot1r = sphere(q,vec3(-.15,0,z-.3), .1);
    float floord = p.y;
